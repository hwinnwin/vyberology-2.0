/**
 * A3 Smoke Test Script
 *
 * Runs A3 codegen and dumps output to local folder.
 * No CI integration - dev-facing only.
 */

import * as fs from 'fs';
import * as path from 'path';
import { generate } from '../packages/a3/src/codegen';
...
async function runSmoke() {
  console.log('🔧 Running A3 Smoke Test...\n');
  ...
  try {
    ...
    const outputPath = path.join(OUTPUT_DIR, 'a3-output.json');
    fs.writeFileSync(outputPath, JSON.stringify(output, null, 2));
    console.log(`💾 Output saved: ${outputPath}`);

    // Write individual files (for inspection)
    const filesDir = path.join(OUTPUT_DIR, 'a3-files');
    ...
    console.log('─'.repeat(40));
    console.log('\n✨ A3 Smoke Test PASSED\n');

    return output;
  } catch (error) {
    console.error('❌ A3 Smoke Test FAILED');
    console.error(error);
    process.exit(1);
  }
}
